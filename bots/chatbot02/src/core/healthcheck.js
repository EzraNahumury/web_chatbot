...
// Mount router under BASE_PATH (e.g. "/chatbot01"). Empty BASE_PATH = root.
if (BASE_PATH) {
  // Redirect "/chatbot01" → "/chatbot01/" so <base href> resolves correctly
...
function startHealthServer() {
  app.listen(PORT, () => {
    logger.info(
      { port: PORT, basePath: BASE_PATH || "/" },
      `Health check server running — open http://localhost:${PORT}${BASE_PATH || ""}`
    );
  });
}
